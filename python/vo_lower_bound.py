...
def click_event_feature_win(event, x, y, flags, params):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        drawImagePatch(FEATURE_IMAGE_PATCH_WINDOW_NAME, prev_image, x, y)
...
def drawImagePatch(window_name, image, x, y):
    patch_size = 30
...
def runOnce(prev_image_idx, prev_image, curr_image):
    ...
    global MOUSE_POINT
    cv2.setMouseCallback(FEATURE_WINDOW_NAME, click_event_feature_win)
    ...
    while len(feature_dict) < goal_feature_cnt:
        ...
        if input_key == 13:  # 'ENTER'
            ...
            try :
                STARTING_FEATURE_IDX = int(starting_feature_num)
            except ValueError:
...
def displayPrevImage(frame_feature_dict, prev_image):
    for feature_idx in frame_feature_dict.keys():
        feature_point = frame_feature_dict[feature_idx]
