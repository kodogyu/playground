...
def train(config):
    ...
    for epoch in range(config.num_epochs):
        ...
        wandb.log({"Training loss": train_loss.item(), "epoch": epoch+1})

        # test accuracy & loss
        test_acc, test_loss = get_test_acc(model=model, dataloader=test_dataloader)
        wandb.log({"Test accuracy": test_acc, "epoch": epoch+1})
        wandb.log({"Test loss": test_loss.item(), "epoch": epoch+1})
