...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
    {
        ...
        std::string match_filename = "/home/kodogyu/github_repos/ORB_SLAM3/my_dir/matches/match_" + std::to_string(pKF->mnId) + "_" + std::to_string(F.mnId) + ".txt";
        match_file.open(match_filename);

        std::stringstream ss;
        std::vector<std::vector<int>> match_vectors;
                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    if(bestDist1<=TH_LOW)
                    {
                        if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                        {
                            ...
                            std::vector<int> match_vec = {(int)realIdxKF, bestIdxF, bestDist1};
                            match_vectors.push_back(match_vec);
                        }
        ...
        match_file << "(ORBmatcher::SearchByBoW) vnMatches12. 'i1 i2 distance'" << std::endl;
        for (int i = 0; i < match_vectors.size(); i++) {
            std::vector<int> match_elem = match_vectors[i];

            if (vpMapPointMatches[match_elem[1]]) {
                ss << match_elem[0] << " " << match_elem[1] << " " << match_elem[2] << std::endl;
            }
        }
        match_file << ss.str();
        match_file.close();
        std::cout << "ORBmatcher::SearchByBoW << [frame " << pKF->mnId << ", " << F.mnId << "] number of matches: " << nmatches << ". " << match_filename << std::endl;
    ...
    int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
    {
        ...
        std::ofstream match_file;
        std::string match_filename = "/home/kodogyu/github_repos/ORB_SLAM3/my_dir/matches/match_" + std::to_string(F1.mnId) + "_" + std::to_string(F2.mnId) + ".txt";
        match_file.open(match_filename);

        std::vector<std::string> vMatch_text;
        ...
        for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
        {
            ...
            if(bestDist<=TH_LOW)
            {
                if(bestDist<(float)bestDist2*mfNNratio)
                {
                    ...
                    // std::stringstream ss;
                    // ss << i1 << " " << bestIdx2 << std::endl;
                    // vMatch_text.push_back(ss.str());
        ...
        match_file << "(ORBmatcher::SearchForInitialization) vnMatches12. 'i1 i2 dist'" << std::endl;
        for (int i = 0; i < vnMatches12.size(); i++) {
            match_file << i << " " << vnMatches12[i] << " " << vMatchedDistance[vnMatches12[i]] << std::endl;
        }
        match_file.close();
        std::cout << "number of matches: " << nmatches << ". " << match_filename << std::endl;
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        ...
        std::ofstream match_file;
        std::string match_filename = "/home/kodogyu/github_repos/ORB_SLAM3/my_dir/matches/match_" + std::to_string(LastFrame.mnId) + "_" + std::to_string(CurrentFrame.mnId) + ".txt";
        match_file.open(match_filename);
            ...
            if(pMP)
            {
                if(!LastFrame.mvbOutlier[i])
                {
                    ...
                    if(bestDist<=TH_HIGH)
                    {
                        ...
                        match_vectors.push_back(std::vector<int>{i, bestIdx2, bestDist});
        ...
        match_file << "(ORBmatcher::SearchByProjection) mvpMapPoints. i1 i2 dist" << std::endl;
        for (int i = 0; i < match_vectors.size(); i++) {
            std::vector<int> match_elem = match_vectors[i];

            if (CurrentFrame.mvpMapPoints[match_elem[1]]) {
                ss << match_elem[0] << " " << match_elem[1] << " " << match_elem[2] << std::endl;
            }
        }
        match_file << ss.str();
        match_file.close();
        std::cout << "ORBmatcher::SearchByProjection [frame " << LastFrame.mnId << ", " << CurrentFrame.mnId << "] number of matches: " << nmatches << ". " << match_filename << std::endl;
