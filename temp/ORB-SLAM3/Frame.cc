     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    if (mnId < 10) {
        // draw keypoints
        cv::Mat kp_image = imGray.clone();
        cv::drawKeypoints(kp_image, mvKeys, kp_image);
        std::string kp_image_filename = "/home/kodogyu/Pictures/kp_image_" + std::to_string(mnId) + ".png";
        cv::imwrite(kp_image_filename, kp_image);
        std::cout << "keypoint drawn. " << kp_image_filename << std::endl;
    }

    // write keypoints
    ofstream kp_file;
    std::string kp_filename = "/home/kodogyu/github_repos/ORB_SLAM3/my_dir/keypoints/frame" + to_string(mnId) + ".txt";
    kp_file.open(kp_filename);

    kp_file << "kp.angle kp.class_id kp.octave kp.pt kp.reponse kp.size" << std::endl;
    for (auto kp : mvKeys) {
        kp_file << kp.angle << " " << kp.class_id << " " << kp.octave << " " << kp.pt << " " << kp.response << " " << kp.size << std::endl;
    }
    kp_file.close();
    std::cout << "number of keypoint [frame " << mnId << "] " << mvKeys.size() << ". " << kp_filename << std::endl;
